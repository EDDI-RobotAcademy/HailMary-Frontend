...
function mapBadRequest(detail: unknown): SajuError {
  ...
  const hints = issues
    .map((i) => {
      ...
      return { field: root, message: FIELD_MESSAGE[root] ?? `${root} 값을 확인해 주세요.` };
    })
...
export async function postSajuFree(
  ...
  switch (res.error) {
    ...
    case "TIMEOUT":
      return { ok: false, error: { code: "TIMEOUT", message: "연결이 늦어지고 있어요." } };
    case "CALCULATION_FAILED":
      return { ok: false, error: { code: "CALCULATION_FAILED", message: "잠시 후 다시 시도해 주세요." } };
    case "NETWORK":
      return { ok: false, error: { code: "NETWORK", message: "네트워크 연결을 확인해 주세요." } };
    default:
      return { ok: false, error: { code: "UNKNOWN", message: "알 수 없는 오류가 발생했어요." } };
  }
}
